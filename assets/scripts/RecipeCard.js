...
class RecipeCard extends HTMLElement {
  ...
  set data(data) {
    ...
    let articleElem = this.shadowRoot.children[0];  // JOHN - THis is kinda brute forcing it
    // A7. TODO - Set the contents of the <article> with the <article> template given in
    //           cardTemplate.html and the data passed in (You should only have one <article>,
    //           do not nest an <article> inside another <article>). You should use Template
    //           literals (tempalte strings) and element.innerHTML for this.
    articleElem.innerHTML = '<img src="'+data.imgSrc+'" alt="'+data.imgAlt+'">   <p class="title"><a href="'+data.titleLnk +'">'+data.titleTxt+'</a></p>   <p class="organization">'+data.organization+'</p>      <div class="rating"><span>'+data.rating+'</span><img src="/assets/images/icons/5-star.svg" alt="5 stars"><span>('+data.numRatings+')</span></div>         <time>'+data.lengthTime+'</time>   <p class="ingredients">'+data.ingredients+'</p>';
  }
}
